...
static int open_authorize_custom_list_file(char *file_path,int *fd_ptr,unsigned long *file_size_ptr);
static unsigned long get_authorize_custom_list_num(unsigned char *file_cnt_buf);
static unsigned long get_authorize_custom_list_mode_switch(unsigned char *file_cnt_buf);
static int set_authorize_custom_list_buf(AUTHORIZE_CUSTOM_ID list_id,unsigned char *file_cnt_buf,unsigned long buf_num,unsigned char mode_switch_type,unsigned long *real_list_num);
static int analysis_authorize_custom_list_line(unsigned char *p,unsigned long index,AUTHORIZE_CUSTOM_ID q,unsigned char mode_switch);
...
*return:
*/
int get_authorize_custom_list(int *shm_list_id,key_t *shm_list_key,unsigned long *num)
{
    int fd;
    ...
    sprintf(file_path,"%s/%s",SNAM_CFG_DIR,PMC_AUTHORIZE_ACCESS_CUSTOM_FILE_NAME);

    if (SAIL_OK != open_authorize_custom_list_file(file_path,&fd,&file_size))
    {
        error("[Err]Open authorize custom list file error.\n");
        return ERR;
    }
   //printf("file_size = %d\n",file_size);
    ...
    {
        error("[Err]Get authorize custom list file content fail.\n");
        free(file_cnt_buf);
        close(fd);
        return ERR;
    }
    ...
    mode_switch = get_authorize_custom_list_mode_switch(file_cnt_buf);
    //printf("mode switch = %d\n",mode_switch);
    line_num = get_authorize_custom_list_num(file_cnt_buf);
    if(line_num == 0){
		free(file_cnt_buf);
		return ERR;
	}
    g_max_shm_key += SHM_KEY_IVL;
    shm_size = authorize_custom_size*line_num;
    ...
    if (shm_id < 0)
    {
		...
		if(shm_id<0){
			error("create authorize custom list shm fail.");
			free(file_cnt_buf);
        		return ERR;
		}
		DEL_SHM(shm_id);
	       shm_id = shmget(g_max_shm_key,shm_size,IPC_CREAT|IPC_EXCL);
	       if(shm_id <0){
		   	error("create authorize custem list shm fail.");
		   	free(file_cnt_buf);
        		return ERR;
	       }
    }
    ...
    *shm_list_id = shm_id;
    ...
    list_id = (AUTHORIZE_CUSTOM_ID)shmat(shm_id,NULL,0);
    if (!list_id)
    {
        error("attach authorize custom  list shm fail.");
        free(file_cnt_buf);
        DEL_SHM(shm_id);
        return ERR;
    }

    (void)set_authorize_custom_list_buf(list_id,file_cnt_buf,line_num,mode_switch,&read_num);
...
static int open_authorize_custom_list_file(char *file_path,int *fd_ptr,unsigned long *file_size_ptr)
{
    ...
    {
        error("[Err]authorize custom  file don't exist.");
        return(CTL_FILE_NOT_EXIST);
    }

    if ((fd = open(file_path,O_RDONLY | O_CREAT)) < 0)
    {
        error("[Err]authorize custom rules file fail.");     
        return(CTL_FILE_OPEN_FAIL);
    }

    if (0 == (file_size = get_file_size(file_path)))
    {  
        error("[Err]authorize custom file no content.");
        close(fd);
...
static unsigned long get_authorize_custom_list_num(unsigned char *file_cnt_buf)
{
   ...
   {
            	  error("get authorize custom list num  err.\n");
                return 0;
   }
    num = atoi(key_val);
    return num;
...
static unsigned long get_authorize_custom_list_mode_switch(unsigned char *file_cnt_buf)
{
   ...
   {
            	  error("getauthorize custom mode switch  err.\n");
                return 1;
   }
   if(strlen(key_val)<2)
   	return 1;
...
static int set_authorize_custom_list_buf(AUTHORIZE_CUSTOM_ID list_id,unsigned char *file_cnt_buf,unsigned long buf_num,unsigned char mode_switch_type,unsigned long *real_list_num)
{
	...
	if (GET_CFG_VAL_FAIL == cfg_get_key_val(s,LIST_INFO_KEY,info_str,key_val))
   	{
            	  error("get authorize custom line  err.\n");
                continue;
   	}
	if(0 == analysis_authorize_custom_list_line(key_val,j,list_id,mode_switch_type)){
  		printf("LINE %d error \n",i);
...
static int analysis_authorize_custom_list_line(unsigned char *p,unsigned long index,AUTHORIZE_CUSTOM_ID q,unsigned char mode_switch){
	...
	AUTHORIZE_CUSTOM_CONTENT_ID content_id = NULL;
	int shm_id =-1;
	if(p==NULL)
		return 0;
	/*get authorize  id*/
...
#endif
	q[index].custom_num= atoi(tmp_addr);
	if(q[index].custom_num ==0)
		return 0;
	g_max_shm_key += SHM_KEY_IVL;
    	shm_size = authorize_custom_content_size*q[index].custom_num;
    	...
    	content_id = (AUTHORIZE_CUSTOM_CONTENT_ID)shmat(shm_id,NULL,0);
    	if (!content_id)
    	{
      	  	error("attach authorize custom  list shm fail.");
        	DEL_SHM(shm_id);
